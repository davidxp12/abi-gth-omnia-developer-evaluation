...
public class CreateSaleHandler : IRequestHandler<CreateSaleCommand, CreateSaleResult>
{
	...
	public async Task<CreateSaleResult> Handle(CreateSaleCommand command, CancellationToken cancellationToken)
	{
		...
		var sale = new SaleEntity
		{
			...
			Products = command.Products.Select(x => this.ApplyRulesSale(command, x)).ToList()
		};
	...
	private ProductSale ApplyRulesSale(CreateSaleCommand createSaleCommand, CreateProductSaleDto createProductSaleDto)
	{
		this.ValidateProductQuantity(createSaleCommand.Customer, createProductSaleDto.Quantity);

		var productSale = new ProductSale
		{
			Name = createProductSaleDto.Name,
			Quantity = createProductSaleDto.Quantity,
			UnitPrice = createProductSaleDto.UnitPrice
		};

		this.ApplyDiscount(productSale);
